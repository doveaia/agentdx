...
description: Master natural language code search with agentdx
---

## Semantic Search

`agentdx search` enables natural language searches across your codebase using vector embeddings. Instead of exact text matching, it understands the *meaning* of your queries.
...
# Limit results
agentdx search "error handling" --limit 5

# JSON output for AI agents (--compact saves ~80% tokens)
agentdx search "database queries" --json --compact
```
...
```bash
agentdx search "authentication" --json           # Full JSON output
agentdx search "authentication" --json --compact # Minimal JSON (no content field)
```
...
agentdx provides two optional search improvements:
...
See [Search Boost](/agentdx/search-boost/) for configuration.
...
See [Hybrid Search](/agentdx/hybrid-search/) for configuration.

### Troubleshooting

| Problem | Solution |
|---------|----------|
| No results | Ensure `agentdx watch` is running and index is built |
| Poor relevance | Try more descriptive queries, check embedder model |
| Missing files | Check `.agentdx/config.yaml` ignore patterns |
...
agentdx search "user authentication logic"
...
agentdx search "database connection and queries"

# Locate API endpoints
agentdx search "REST API route handlers"
```
...
agentdx search "payment processing" --json --compact --limit 5
```

### Commands Reference

- [`agentdx search`](/agentdx/commands/agentdx_search/) - Full CLI reference
