---
title: File Watching
description: Real-time index updates with agentdx watch
---

## File Watching

`agentdx watch` is the core daemon that maintains your codebase index. It performs an initial full scan and then monitors for file changes in real-time.
...
- **Symbol extraction**: Builds call graph data for `agentdx trace`
- **Debounced processing**: Batches rapid changes efficiently

### Quick Start

```bash
# Initialize agentdx in your project
agentdx init
...
```
Starting agentdx watch in /path/to/project
...
2. **Config ignore patterns**: From `.agentdx/config.yaml`
...
- **Used by**: `agentdx trace` for call graph analysis

See [Call Graph Analysis](/agentdx/trace/) for more details.

### Configuration

Configure watcher behavior in `.agentdx/config.yaml`:
...
- **Location**: `.agentdx/index.gob` (or PostgreSQL)
...
```bash
# Using tmux
tmux new-session -d -s agentdx 'agentdx watch'

# Using screen
screen -dmS agentdx agentdx watch

# Using nohup
nohup agentdx watch > agentdx.log 2>&1 &
```
...
agentdx search "current feature implementation"
```
...
agentdx search "security vulnerabilities" --json --compact
```

### Commands Reference

- [`agentdx watch`](/agentdx/commands/agentdx_watch/) - Full CLI reference
- [`agentdx status`](/agentdx/commands/agentdx_status/) - Check index status
- [`agentdx init`](/agentdx/commands/agentdx_init/) - Initialize configuration
