...
description: Analyze function relationships with agentdx trace
---

## Call Graph Analysis

`agentdx trace` provides call graph analysis for your codebase, allowing you to understand how functions relate to each other by tracking callers and callees.
...
```bash
agentdx trace callers "MyFunction" --mode fast
```
...
```bash
agentdx trace callers "MyFunction" --mode precise
```
...
Configure trace behavior in `.agentdx/config.yaml`:
...
1. **Symbol Indexing**: During `agentdx watch`, symbols (functions, methods, classes) are extracted from source files
...
4. **Persistent Storage**: Symbols are stored in `.agentdx/symbols.gob`
...
agentdx trace callers "ValidateToken"

# What does this function depend on?
agentdx trace callees "ProcessPayment"
```
...
agentdx trace graph "DatabaseConnect" --depth 4
```
...
agentdx trace graph "AuthMiddleware" --depth 2 --json
```

### Commands Reference

- [`agentdx trace callers`](/agentdx/commands/agentdx_trace_callers/) - Find functions that call a symbol
- [`agentdx trace callees`](/agentdx/commands/agentdx_trace_callees/) - Find functions called by a symbol
- [`agentdx trace graph`](/agentdx/commands/agentdx_trace_graph/) - Build complete call graph
