...
2. Explore the codebase using agentdx search to understand related code
...
This creates the binary at `./bin/agentdx`.
...
./bin/agentdx <command-to-test>

# Examples:
# - New CLI command: ./bin/agentdx new-command --flag
# - Search fix: ./bin/agentdx search "test query"
# - Trace feature: ./bin/agentdx trace callers "FunctionName"
# - Config change: ./bin/agentdx init && cat .agentdx/config.yaml
```

**You MUST:**

- Build the CLI binary with `make build`
- Execute the CLI binary (`./bin/agentdx`) to test your changes
...
- **Use agentdx** for code exploration (as per project guidelines)
